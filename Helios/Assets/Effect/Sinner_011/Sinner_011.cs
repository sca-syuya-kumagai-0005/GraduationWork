...
public sealed class Sinner_011 : MonoBehaviour
{
    ...
    private IEnumerator ActionRoutine()
    {
        ...
        Coroutine fov = StartCoroutine(FOVRoutine(true));
        yield return move;
        yield return fov;
        ...
        StartCoroutine(FOVRoutine(false));
    }
    ...
    private IEnumerator FOVRoutine(bool forward)
    {
        if (mainCamera == null) yield break;

        float from = forward ? startFOV : endFOV;
        float to = forward ? endFOV : startFOV;

        float t = 0f;
        while (t < fovDuration)
        {
            t += Time.deltaTime;
            float n = Mathf.Clamp01(t / fovDuration);
            float curve = fovCurve.Evaluate(n);
            mainCamera.fieldOfView = Mathf.Lerp(from, to, curve);
            yield return null;
        }
